...
Template["appBody"] = new Template("Template.appBody", (function() {
  ...
  return HTML.DIV({
    ...
    return [ "\n        ", HTML.DIV({
      ...
      return [ "\n            ", HTML.A({
        "class": "js-details btn-secondary"
      }, "Account Details"), "\n            ", HTML.A({
    ...
    return [ "\n      ", HTML.DIV({
      ...
      "class": "js-new-cast link-list-new"
    }, HTML.SPAN({
      "class": "icon-plus"
    }), "Broadcast"), "\n\n     ", HTML.Comment('    {{#each lists}}\n          <a href="{{pathFor \'listsShow\'}}" class="list-todo {{activeListClass}}" title="{{name}}">\n            {{#if userId}}\n              <span class="icon-lock"></span>\n            {{/if}}\n            {{#if incompleteCount}}\n              <span class="count-list">{{incompleteCount}}</span>\n            {{/if}}\n            {{name}}\n          </a>\n        {{/each}} '), "\n        ", HTML.Comment(' {{#each broadcasts}}\n          <a href="{{pathFor \'castsShow\'}}" class="list-todo {{activeListClass}}" title="{{place}}">\n            {{place}} - {{castername}}\n          </a>\n        {{/each}} '), "\n\n      "), "\n      " ];
